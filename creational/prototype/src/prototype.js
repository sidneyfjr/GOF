/*
  ...
  Ao criar instâncias do protótipo, como `prototypeInstance`, você pode cloná-las para criar 
  novos objetos,   como `clone1` e `clone2`. Cada clone pode ter seu próprio valor modificado, 
  enquanto compartilha a mesma estrutura básica do protótipo original.

  O padrão Prototype é particularmente útil quando você precisa criar várias instâncias
  de um objeto com configurações iniciais semelhantes ou quando o processo 
  de criação de objetos é complexo e pesado. Isso ajuda a melhorar a eficiência 
